...
__global__ void gol_kernel(const WORD_TYPE* input, WORD_TYPE* output, int dim) {
    ...
    for (int bit = 1; bit < sizeof(uint64_t) * 8 - 1; bit++) {
        ...
        WORD_TYPE top_neighbors = (tc >> (bit - 1)) & 7;
        WORD_TYPE center_neighbors = (cc >> (bit - 1)) & 5;
        WORD_TYPE bottom_neighbors = (bc >> (bit - 1)) & 7;

        WORD_TYPE neighbors = (top_neighbors << 6) | (center_neighbors << 3) | bottom_neighbors;

        int count = __popcll(neighbors);
    ...
    // 63rd bit
    {
        ...
        count += __popcll(tc >> 62);
        count += (cc >> 62) & 1;
        count += __popcll(bc >> 62);
